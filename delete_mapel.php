...
if (isset($_POST["id_matapelajaran"]) && !empty($_POST["id_matapelajaran"])) {
    ...
    $sql = "DELETE FROM mata_pelajaran WHERE id_matapelajaran = :id_matapelajaran";

    if ($stmt = $pdo->prepare($sql)) {
        $stmt->bindParam(':id_matapelajaran', $param_id_mapel);
        $param_id_mapel = trim($_POST["id_matapelajaran"]);
        if ($stmt->execute()) {
    ...
    if (empty(trim($_GET["nama_mapel"]))) {
        header("location: error_mapel.php");
        exit();
    }
}
?>
                ...
                <div id="content">
                    ...
                    <div class="container-fluid">
                        <div class="d-sm-flex align-items-center justify-content-between mb-4">
                            <h1 class="h3 mb-0 text-gray-800">Delete Mapel</h1>
                        </div>
                                ...
                                <div class="row">
                                    <div class="col-md-12">
                                        ...
                                        <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>"
                                            method="post">
                                            <div>
                                                <input type="hidden" name="nama_mapel"
                                                    value="<?php echo trim($_GET["nama_mapel"]); ?>">
                                                <input type="hidden" name="id_matapelajaran"
                                                    value="<?php echo trim($_GET["id_matapelajaran"]); ?>">
                                                <p>apakah anda akan meng-delete data ini?</p><br>
                                                <p>
                                                    <input type="submit" value="YA" class="btn btn-danger">
                                                    <a href="mapel.php" class="btn btn-primary">Tidak</a>
                                                </p>
                                            </div>
                                        </form>
                                    </div>
